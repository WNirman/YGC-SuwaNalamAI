...
import { MOCK_EXTRACTED_DATA, MOCK_TIMELINE, MOCK_PATIENT } from '@/lib/mockData';
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        // Ensure the extracted data has the file reference
      ...
      return NextResponse.json(
        {
          success: false,
          error: isKeyError 
            ? 'Your Gemini API Key is invalid or expired. Please click "Reset API / Exit Demo" in the sidebar and enter a valid key from Google AI Studio.' 
    ...
    // Merge patient info (take the most complete version)
